...
description: Sometimes we need to plot the predicted probability of a binary outcome. For example, using the famous iris data set...
categories: []
tags: []
draft: false
...
X%>%
  mutate(Setosa=prob,
         `Not Setosa`=1-prob)%>%
  tidyr::gather(outcome,p,Setosa, `Not Setosa`)%>%
  mutate(ny=ifelse(outcome=="Setosa",p-0.05,(1-p)+0.05))%>%
  ggplot(aes(x=id,y=p,fill=outcome,label=paste0(round(p*100),"%")))+geom_col()+
  coord_flip()+geom_text(aes(y=ny))+
  theme(axis.title.y = element_text(angle=0))
```
